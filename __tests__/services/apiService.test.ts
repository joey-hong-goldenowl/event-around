...
jest.mock('../../src/utils/apiService', () => ({
  foursquareResponseMapper: jest.fn(),
  tickermasterResponseMapper: jest.fn(),
  yelpResponseMapper: jest.fn(),
}));
...
describe('apiService', () => {
  ...
  test('should fetch and combine events from all sources using mock data', async () => {
    const mockFoursquareEvents = [
      {
    ...
    (foursquareResponseMapper as jest.Mock).mockReturnValue(
      mockFoursquareEvents,
    );
    (tickermasterResponseMapper as jest.Mock).mockReturnValue(
      mockTicketmasterEvents,
    );
    (yelpResponseMapper as jest.Mock).mockReturnValue(mockYelpEvents);

    const result = await getEvents();

    expect(foursquareResponseMapper).toHaveBeenCalledWith(foursquareJson);
    expect(tickermasterResponseMapper).toHaveBeenCalledWith(ticketmasterJson);
    expect(yelpResponseMapper).toHaveBeenCalledWith(yelpJson);
  ...
  test('should handle empty JSON data', async () => {
    (foursquareResponseMapper as jest.Mock).mockReturnValue([]);
    (tickermasterResponseMapper as jest.Mock).mockReturnValue([]);
    (yelpResponseMapper as jest.Mock).mockReturnValue([]);
  ...
  test('should simulate async delay', async () => {
    ...
    (foursquareResponseMapper as jest.Mock).mockReturnValue([]);
    (tickermasterResponseMapper as jest.Mock).mockReturnValue([]);
    (yelpResponseMapper as jest.Mock).mockReturnValue(mockEvents);
